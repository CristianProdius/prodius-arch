...
router.post("/api/projects/delete", async ({ request, user }) => {
  try {
    ...
    const userKey = `${PROJECT_PREFIX}${id}`;
    await userPuter.kv.del(userKey);
...
router.post("/api/projects/rename", async ({ request, user }) => {
  try {
    ...
    const body = await request.json();
    const { id, name } = body || {};
    if (!id || !name) return jsonError(400, "Project id and name required");

    const userId = await getUserId(userPuter);
    if (!userId) return jsonError(401, "User id required");

    const userKey = `${PROJECT_PREFIX}${id}`;
    const project = await userPuter.kv.get(userKey);

    if (project) {
      project.name = name;
      project.updatedAt = new Date().toISOString();
      await userPuter.kv.set(userKey, project);
      return jsonOk({ renamed: true, id, project });
    }

    const publicKey = getPublicKey(userId, id);
    const publicProject = await mePuter.kv.get(publicKey);
    if (publicProject) {
      publicProject.name = name;
      publicProject.updatedAt = new Date().toISOString();
      await mePuter.kv.set(publicKey, publicProject);
      return jsonOk({ renamed: true, id, project: publicProject });
    }

    return jsonError(404, "Project not found");
  } catch (error) {
    return jsonError(500, "Failed to rename project", {
      message: error?.message || "Unknown error",
    });
  }
});

router.post("/api/projects/duplicate", async ({ request, user }) => {
  try {
    const userPuter = user.puter;
    if (!userPuter) return jsonError(401, "Authentication required");
    ...
    const clone = {
      ...project,
      id: newId,
      name: (project.name || `Residence ${id}`) + " (Copy)",
      timestamp: Date.now(),
      updatedAt: new Date().toISOString(),
      isPublic: false,
    };

    const newKey = `${PROJECT_PREFIX}${newId}`;
    await userPuter.kv.set(newKey, clone);

    return jsonOk({ duplicated: true, id: newId, project: clone });
  } catch (error) {
    return jsonError(500, "Failed to duplicate project", {
      message: error?.message || "Unknown error",
    });
  }
});

router.post("/api/projects/batch-delete", async ({ request, user }) => {
  try {
    const mePuter = me.puter;
    const userPuter = user.puter;
    if (!userPuter) return jsonError(401, "Authentication required");
    ...
    const body = await request.json();
    const ids = body?.ids;
    if (!Array.isArray(ids) || ids.length === 0) return jsonError(400, "Project ids required");
    ...
    await Promise.all(
      ids.map(async (id) => {
        await userPuter.kv.del(`${PROJECT_PREFIX}${id}`);
        await mePuter.kv.del(getPublicKey(userId, id));
      }),
    );

    return jsonOk({ deleted: true, count: ids.length });
  } catch (error) {
    return jsonError(500, "Failed to batch delete", {
      message: error?.message || "Unknown error",
    });
  }
});

router.post("/api/projects/update-tags", async ({ request, user }) => {
  try {
    const mePuter = me.puter;
    const userPuter = user.puter;
    if (!userPuter) return jsonError(401, "Authentication required");
    ...
    const body = await request.json();
    const { id, tags } = body || {};
    if (!id || !Array.isArray(tags)) return jsonError(400, "Project id and tags required");
    ...
    if (project) {
      ...
      project.updatedAt = new Date().toISOString();
      await userPuter.kv.set(userKey, project);
      return jsonOk({ updated: true, id, project });
    }

    const publicKey = getPublicKey(userId, id);
    const publicProject = await mePuter.kv.get(publicKey);
    if (publicProject) {
      publicProject.tags = tags;
      publicProject.updatedAt = new Date().toISOString();
      await mePuter.kv.set(publicKey, publicProject);
      return jsonOk({ updated: true, id, project: publicProject });
    }

    return jsonError(404, "Project not found");
  } catch (error) {
    return jsonError(500, "Failed to update tags", {
      message: error?.message || "Unknown error",
    });
  }
});
...
router.post("/api/projects/clear", async ({ user }) => {
  ...
  const publicDeleted = mePuter?.kv
    ? await deleteKvByPattern(mePuter.kv, `${PUBLIC_PREFIX}${userId}_*`)
    : 0;
